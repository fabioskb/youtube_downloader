...
public class YoutubeEventosMenores extends YoutubeEventosPesquisa {
    ...
    protected void btnCancelarClick(ActionEvent ev) {
        if (getDownloadProgressBar().isVisible() || getLblResultado().getText().startsWith(TEXTOS.pegarTexto("label.resultado.verificando.download")) || getLblResultado().getText().equals(TEXTOS.pegarTexto("label.resultado.pesquisa.concluida"))) {
            getPro().destroy();
