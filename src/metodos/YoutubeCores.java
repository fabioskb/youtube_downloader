...
public class YoutubeCores {
    ...
    /**
     * DEPRECATED Cores para a aplicação, é aconselhável usar um método para
     * setar o modo noturno, e então usa-lo como um boolean getter no argumento
     * noite para uma melhor automoção.
     *
     * @param noite (boolean) - Se true o modo noturno será aplicado para o
     * componente, se false, o padrão será aplicado.
     * @param indiceCor (int) - [0] Banner BG e Descrição Borda, [1] Banner FG,
     * [2] lblResultado BG, [3] Padrão BG, [4] Descrição BG, [5] Botões BG, [6]
     * FG botões e descrição, [7] Erros, [8] Avisos, [9] Concluído com sucesso,
     * [10] Área de texto (link, pesquisa e lista), [11] lblProgressBar e
     * downloadProgressBar FG.
     * @return Retorna uma cor de cores[].
     */
    public Color pegarCor(boolean noite, int indiceCor) {
        if (noite) {
            return Color.decode(coresNight[indiceCor]);
        } else {
            return Color.decode(cores[indiceCor]);
        }

    }

    /**
     * Cores para a aplicação, é aconselhável usar um método para setar o modo
     * noturno, e então usa-lo como um boolean getter no argumento noite para
     * uma melhor automoção.
     *
     * @param noite (boolean) - Se true o modo noturno será aplicado para o
     * componente, se false, o padrão será aplicado.
     * @param key (String) - Banner BG e Descricao borda, Banner FG,
     * lblResultado BG, Padrao BG, Descricao BG, Botoes BG, Botoes e descricao
     * FG, Erros, Avisos, Concluido, Area de texto, lblProgressBar e
     * downloadProgressBar FG.
     * @return Retorna uma cor.
     */
    public Color pegarCor(boolean noite, String key) {
        if (noite) {
            return Color.decode(nightColors.get(key));
        } else {
     ...
     * @return Retorna uma cor.
     */
    public Color pegarCorComBrilho(boolean noite, String key) {
        if (noite) {
            return Color.decode(nightColors.get(key)).brighter();
        } else {
            return Color.decode(colors.get(key)).darker();
        }
